...
const TableHeader: React.FC<TableHeader> = ({
  ...
  <Flex alignItems="center" justifyContent="space-between">
    ...
    <Flex alignItems="center">
      <Button mr={3} icon="AddCircleOutline" mainColor="#25D78F" onClick={() => openAddModal()} disabled={disabledAdd}>
        Add Whitelisted Node
      </Button>
      {!isReadOnly && !pendingLock && (
        <Tooltip message="Prevent any changes to the contracts." placement="bottom" variant="dark">
          <Button.Outline
            icon="LockOpen"
            mainColor="black"
      ...
      {isReadOnly && !pendingLock && (
        <Tooltip message="Unlock to make changes." placement="bottom" variant="dark">
          <Button variant="danger" icon="Lock" onClick={() => openLockModal()} disabled={disabledLock}>
            Allow Changes
          </Button>
        </Tooltip>
      )}
