...
const LockModal: React.FC<{
  ...
  <Modal isOpen={isOpen}>
    <Card width={'700px'} p={0}>
      ...
      />
      <Box p={4} mb={3}>
        <Heading.h3>{isReadOnly ? 'Unlock and allow changes?' : 'Lock values?'}</Heading.h3>
        <Text>
          {isReadOnly ? 'Are you sure you want to allow changes' : 'Are you sure you want to prevent any changes?'}
        </Text>
      </Box>
