...
contract("NodeRules (Permissioning)", (accounts) => {
  ...
  it('should NOT permit node when list is empty', async () => {
    ...
    let permitted = await nodeRulesContract.enodePermitted(node1High, node1Host, node1Port);
  ...
  it('Should NOT fail when removing enode from empty list', async () => {
    ...
    let tx = await nodeRulesContract.removeEnode(node1High, node1Host, node1Port);
    assert.ok(tx.receipt.status);
  });

  it('should add multiple nodes to list', async () => {
    await nodeRulesContract.addEnode(node1High, node1Host, node1Port);
    await nodeRulesContract.addEnode(node2High, node2Host, node2Port);
    await nodeRulesContract.addEnode(node3High, node3Host, node3Port);

    permitted = await nodeRulesContract.enodePermitted(node1High, node1Host, node1Port);
    assert.ok(permitted, 'expected node 1 added to be in list');

    permitted = await nodeRulesContract.enodePermitted(node2High, node2Host, node2Port);
    assert.ok(permitted, 'expected node 2 added to be in list');

    permitted = await nodeRulesContract.enodePermitted(node3High, node3Host, node3Port);
  ...
  it('should allow a connection between nodes added to the list', async () => {
    let permitted = await nodeRulesContract.connectionAllowed(node1High, node1Host, node1Port, node2High, node2Host, node2Port);
    assert.equal(permitted, PERMITTED, 'expected permitted node1 <---> node2');

    permitted = await nodeRulesContract.connectionAllowed(node1High, node1Host, node1Port, node3High, node3Host, node3Port);
    assert.equal(permitted, PERMITTED, 'expected permitted node1 <---> node3');

    permitted = await nodeRulesContract.connectionAllowed(node2High, node2Host, node2Port, node3High, node3Host, node3Port);
  ...
  it('should NOT allow connection with node removed from list', async () => {
    await nodeRulesContract.removeEnode(node3High, node3Host, node3Port);
    let permitted = await nodeRulesContract.enodePermitted(node3High, node3Host, node3Port);
    assert.notOk(permitted, 'expected removed node NOT permitted');

    permitted = await nodeRulesContract.connectionAllowed(node3High, node3Host, node3Port, node2High, node2Host, node2Port);
  ...
  it('should not allow non-admin account to add node to list', async () => {
    try {
      await nodeRulesContract.addEnode(node1High, node1Host, node1Port, { from: accounts[1] });
  ...
  it('should allow new admin account to remove node from list', async () => {
    ...
    await nodeRulesContract.removeEnode(node1High, node1Host, node1Port, { from: accounts[1] });
  ...
  it('should allow new admin account to add node to list', async () => {
    ...
    await nodeRulesContract.addEnode(node1High, node1Host, node1Port, { from: accounts[2] });
