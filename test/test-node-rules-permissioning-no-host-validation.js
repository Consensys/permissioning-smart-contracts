...
contract("NodeRules (Permissioning without Host)", (accounts) => {
  ...
  it('should allow permit node when list is empty', async () => {
  ...
  it('should add node to list', async () => {
  ...
  it('should add node more nodes to list', async () => {
  ...
  it('should permitted node with no host validation', async () => {
    permitted = await nodeRulesContract.enodePermitted(enode1, node1Host, node1Port);
    assert.ok(permitted, 'expected node 1 added to be in list');

    permitted = await nodeRulesContract.enodePermitted(enode1, node2Host, node1Port);
    assert.ok(permitted, 'expected node 1 added to be in list');
  });
  ...
  it('should allow to activate the validation of the Host', async () => {
    await nodeRulesContract.activateHostValidation(true);

    let permitted = await nodeRulesContract.enodePermitted(enode1, node1Host, node1Port);
    assert.ok(permitted, 'expected node 1 permitted');

    permitted = await nodeRulesContract.enodePermitted(enode2, node2Host, node2Port);
    assert.ok(permitted, 'expected node 2 permitted');

    permitted = await nodeRulesContract.enodePermitted(enode3, node3Host, node3Port);
    assert.ok(permitted, 'expected node 3 permitted');

  });

  it('should validate host after the validation of the Host', async () => {
    let permitted = await nodeRulesContract.enodePermitted(enode1, node1Host, node1Port);
    assert.ok(permitted, 'expected added node permitted');
    permitted = await nodeRulesContract.enodePermitted(enode1, node2Host, node1Port);
    assert.notOk(permitted, 'expected added node NOT permitted');
