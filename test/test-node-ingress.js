...
contract ("Node Ingress (no contracts registered)", (accounts) => {
    ...
    it("should forbid any connection if rules contract has not been registered", async () => {
        ...
        let permitted = await nodeIngressContract.connectionAllowed(nodeHigh, nodeHost, nodePort, node2High, node2Host, node2Port);
...
contract("Ingress contract", (accounts) => {
    ...
    it("Should emit an event when the NodeRules are updated", async () => {
        ...
        await nodeRulesContract.addEnode(nodeHigh, nodeHost, nodePort, { from: accounts[0] });
        ...
        result = await nodeRulesContract.removeEnode(nodeHigh, nodeHost, nodePort);
